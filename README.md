...
- `Next.js` backend routes / server actions
- `Prisma`
- `PostgreSQL`
- `NextAuth` or custom JWT authentication
...
- Pakistan-supported payment integrations:
  - `Cash on Delivery`
  - `JazzCash`
  - `Easypaisa`
  - `Local card gateway`
...
Next.js allows the storefront and the dashboard experiences to live in one modern web stack. It supports SEO, server-side rendering, fast page loads, and clean route-based separation for customers, vendors, and admins.
...
These are a strong combination for building responsive pages quickly, especially for admin/vendor dashboards, product grids, forms, tables, modals, and analytics layouts.
...
PostgreSQL is a strong fit for relational commerce data and reporting. Prisma provides a clean developer experience on top of it.
...
The storefront will launch in `English` and `Urdu`, so a proper i18n layer is necessary for translated UI labels and localized content delivery.
...
4. Platform creates order and payment record
5. Vendor receives order
6. Vendor prepares order
7. Vendor marks order ready for pickup or out for delivery
8. Order completes and commission is recorded
